...
import { InsertUser, users, ContactSubmission, InsertContactSubmission, contactSubmissions } from "../drizzle/schema";
...
export async function createContactSubmission(
  submission: Omit<InsertContactSubmission, 'createdAt' | 'updatedAt'>
  ...
  try {
    ...
    if (result.length > 0) {
      const created = await db
        ...
        .where(eq(contactSubmissions.id, result[0].id))
        .limit(1);
